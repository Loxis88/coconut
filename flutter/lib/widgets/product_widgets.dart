...
String _resolveImageUrl(String url) => _needsProxy(url)
    ? '$coconutBackendBaseUrl/proxy/image?url=${Uri.encodeComponent(url)}'
    : url;
  ...
  Widget build(BuildContext context) => Column(
        children: [
          Row(children: [
            ...
            NutrientCell(
                label: 'Углеводы', value: nutrients.carbohydrates ?? '-'),
          ]),
          Row(children: [
            NutrientCell(label: 'Ккал', value: nutrients.calories ?? '-'),
            if (nutrients.fiber != null)
              NutrientCell(label: 'Клетчатка', value: nutrients.fiber!),
          ]),
        ],
      );
}
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            Text('$value', style: TextStyle(color: tier.color))
          ]),
          ...
          TweenAnimationBuilder<double>(
            ...
            builder: (context, v, _) => LinearProgressIndicator(
                value: v, color: tier.color, backgroundColor: MayakTheme.muted),
          ),
          Text(note,
              style: const TextStyle(color: MayakTheme.mutedFg, fontSize: 12)),
        ],
      ),
    );
  }
...
class Flag extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: CocoCard(
          child: Row(children: [
            const CircleAvatar(
                backgroundColor: MayakTheme.scoreExcellent,
