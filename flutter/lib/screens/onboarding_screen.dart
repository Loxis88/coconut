...
final _slides = [
  OnboardingSlide(
    ...
    heading: 'Понимайте\nчто вы едите',
    body:
        'МАЯК расшифровывает этикетки продуктов и объясняет состав простым и честным языком — без химического жаргона.',
  ...
  OnboardingSlide(
    ...
    heading: 'Сканируйте\nза секунду',
    body:
        'Наведите камеру на штрихкод — и получите полный научный анализ продукта до того, как положите его в корзину.',
  ...
  OnboardingSlide(
    ...
    heading: 'Научная\nоснова',
    body:
        'Каждая оценка опирается на рецензируемые исследования, критерии ВОЗ и базу данных Open Food Facts.',
  ...
  OnboardingSlide(
    ...
    heading: 'Найдите\nлучшую замену',
    body:
        'Маяк предложит более полезную альтернативу в той же категории и объяснит, чем она лучше.',
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: Padding(
                  padding:
                      const EdgeInsets.only(left: 28, right: 28, bottom: 8),
                  child: AnimatedSwitcher(
              ...
              Padding(
                padding: const EdgeInsets.only(
                    left: 28, right: 28, bottom: 32, top: 16),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    // Dots
                        ...
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              color: isActive
                                  ? slide.ink
                                  : slide.ink.withValues(alpha: 0.25),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(55, 20, 150, 160), const Radius.circular(16)),
        paint);

    paint.color = const Color(0xFF153918).withValues(alpha: 0.12);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(70, 36, 120, 14), const Radius.circular(7)),
        paint);

    paint.color = const Color(0xFF153918).withValues(alpha: 0.07);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(70, 58, 88, 9), const Radius.circular(4.5)),
        paint);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(70, 74, 104, 9), const Radius.circular(4.5)),
        paint);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(70, 90, 72, 9), const Radius.circular(4.5)),
        paint);
    ...
    final tp = TextPainter(
      text: TextSpan(
          ...
          style: GoogleFonts.fraunces(
              ...
              color: const Color(0xD9153918))), // opacity 0.85 approx D9
...
class Visual2Painter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(95, 10, 70, 120), const Radius.circular(12)),
        paint);

    paint.color = Colors.white.withValues(alpha: 0.5);
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(100, 16, 60, 108), const Radius.circular(9)),
        paint);
    ...
    canvas.drawRRect(
        RRect.fromRectAndRadius(
            const Rect.fromLTWH(113, 38, 34, 52), const Radius.circular(4)),
        paint);
    ...
    for (int i = 0; i < corners.length; i++) {
      ...
      final y = corners[i][1];
      canvas.drawLine(
          Offset(x, y), Offset(x + (i % 2 == 0 ? 8 : -8), y), paint);
    ...
    for (int i = 0; i < 8; i++) {
      paint.color =
          const Color(0xFF153918).withValues(alpha: 0.08 + (i % 3) * 0.04);
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTWH(116.0 + i * 4, 46, 2.0 + (i % 3), 36),
    ...
    final tpCenter = TextPainter(
      text: TextSpan(
          ...
          style: GoogleFonts.dmMono(
              fontSize: 13,
              fontWeight: FontWeight.w700,
              color: const Color(0xFF153918).withValues(alpha: 0.6))),
      textDirection: TextDirection.ltr,
    );
    tpCenter.layout();
    tpCenter.paint(
        canvas, Offset(130 - tpCenter.width / 2, 100 - tpCenter.height / 2));
...
class Visual4Painter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var p in products) {
      ...
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTWH(x, 30, 72, 96), const Radius.circular(12)),
          paint);

      paint.color = Colors.white.withValues(alpha: 0.5);
      canvas.drawRRect(
          RRect.fromRectAndRadius(
              Rect.fromLTWH(x + 8, 38, 56, 40), const Radius.circular(8)),
          paint);
      ...
      final tpLabel = TextPainter(
        text: TextSpan(
            ...
            style: GoogleFonts.dmSans(
                fontSize: 10,
                color: const Color(0xFF153918).withValues(alpha: 0.5))),
        textDirection: TextDirection.ltr,
      );
      tpLabel.layout();
    ...
    final tpStar = TextPainter(
      text: TextSpan(
          ...
          style: TextStyle(
              fontSize: 14,
              color: const Color(0xFF1E6B28).withValues(alpha: 0.7))),
      textDirection: TextDirection.ltr,
    );
    tpStar.layout();
